<x-app-layout>
        ...
        <div class="p-4 mt-14">
            ...
            <div x-data="listSampah" class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <div class="flex items-center justify-between px-4 py-4 bg-white dark:bg-gray-800">
                    <!-- Add Item -->
                    <div class="text-sm">
                        <button class="bg-blue-500 font-semibold text-white rounded-md shadow-lg px-2 py-2 flex items-center focus:outline-none focus:ring-offset-1 focus:ring-2 focus:ring-blue-500 transition-all" onclick="toggleModal('tambah-sampah-modal')">
                    ...
                    <div class="text-sm">
                        ...
                        <div class="relative flex rounded-md shadow-sm">
                            <input id="search-input" type="search"  name="query" x-on:keyup.enter="searchSubmit" x-on:input="searchInput"
                                   class="py-2 px-4 pl-11 block w-full border-gray-200 shadow-sm rounded-l-md text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="Cari data tipe sampah">
                        ...
                        <tbody>
                            ...
                            <template x-for="sampah in items" x-cloak>
                                <tr aria-label="table-row" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center" x-text="sampah.harga_satuan">
    ...
    <x-forms.input id="edit-sampah-modal" route="tipe-sampah.edit-data">
        ...
        <div>
            <label for="edit-satuan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Satuan</label>
            <input type="text" name="satuan" id="edit-satuan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
        </div>
        <div>
            <label for="edit" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Harga Satuan</label>
            <input type="text" name="harga_satuan" id="edit-harga" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
        </div>

    </x-forms.input>

    <!-- Create Item Sampah Modal -->
    <x-forms.input id="tambah-sampah-modal" route="tipe-sampah.submit-data">
        <x-slot:title>
            Tambah Tipe Barang
        </x-slot:title>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('listSampah', () => ({
            ...
            navItems : null,
            async init() {
                let resp = await fetch('http://localhost:8080/tipe-sampah/data');
            ...
            async searchSubmit(e){
                ...
                let resp = await fetch('http://localhost:8080/tipe-sampah/data?search=' + value);
                const listItems = await resp.json();
            ...
            async navigationTable(value){
                ...
                let resp = await fetch('http://localhost:8080/tipe-sampah/data?page=' + value + searchValue )
                const listItems = await resp.json();
                const nextButton = document.getElementById('next')
