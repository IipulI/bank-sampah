<x-app-layout>
        ...
        <div class="p-4 mt-14">
            ...
            <div x-data="listSampah" class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <div class="flex items-center justify-between px-4 py-4 bg-white dark:bg-gray-800">
                    <!-- Add Item -->
                    <div class="text-sm">
                        <button class="bg-blue-500 font-semibold text-white rounded-md shadow-lg px-2 py-2 flex items-center focus:outline-none focus:ring-offset-1 focus:ring-2 focus:ring-blue-500 transition-all" onclick="toggleModal('tambah-staff-modal')">
                    ...
                    <div class="text-sm w-2/6">
                        ...
                        <div class="relative flex rounded-md shadow-sm">
                            <input id="search-input" type="search"  name="query" x-on:keyup.enter="searchSubmit" x-on:input="searchInput"
                                   class="py-2 px-4 pl-11 block w-full border-gray-200 shadow-sm rounded-l-md text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="Cari nama staff">
                    ...
                    <table id="sampahTable" class="px-4 border-b-2 w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <template x-if="items?.length === 0">
                            <tr aria-label="table-row" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="px-6 py-4 text-gray-900 font-semibold text-lg text-center" colspan="3">
                                    Data tidak ditemukan
                                </td>
                            </tr>
                        </template>
                        <template x-for="staff in items" x-cloak>
                            <tr aria-label="table-row" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                                :class="!items ? 'hidden' : ''">
    ...
    <x-forms.input id="tambah-staff-modal" route="staff.submit-data">
        <x-slot:title>
            Tambah Staff
        </x-slot:title>
        ...
        <div>
            <label for="tambah-password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pssword</label>
            <input type="password" name="password" id="tambah-password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required></input>
        </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('listSampah', () => ({
            ...
            navItems : null,
            async init() {
                let resp = await fetch('http://localhost:8080/staff/data');
            ...
            async searchSubmit(e){
                ...
                let resp = await fetch('http://localhost:8080/staff/data?search=' + value);
                const listItems = await resp.json();
            ...
            async navigationTable(value){
                ...
                let resp = await fetch('http://localhost:8080/staff/data?page=' + value + searchValue )
                const listItems = await resp.json();
                const nextButton = document.getElementById('next')
